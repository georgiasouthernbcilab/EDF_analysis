...
def plot_psd(edf_file, output_directory):
    try:
        raw = mne.io.read_raw_edf(edf_file, preload=True, infer_types=True, verbose=True)# Load the EDF/BDF file
        ...
        raw.set_eeg_reference('average', projection=True)# Set EEG average reference and apply band-pass filter
        if apply_proj:
            raw.apply_proj()
        raw.filter(l_freq=1, h_freq=40) # Apply bandpass filter
        # Set up ICA
        ica = ICA(n_components=32, random_state=97, max_iter="auto") 
        ica.fit(raw)

        # Find EOG and muscle artifacts
        eog_indices, eog_scores = ica.find_bads_eog(raw, ch_name=eog_channels) # Define EOG indicies and scores
...
def plot_topomap(edf_file, output_directory):
# def plot_psd(edf_file, output_directory):
    try:
        raw = mne.io.read_raw_edf(edf_file, preload=True, infer_types=True, verbose=True)# Load the EDF/BDF file
        raw.pick(eeg_channels)# Pick only EEG channels
        ...
        for i, event in enumerate(events):
            ...
            # Save the plot to a PNG file
            output_filename = f"{os.path.basename(edf_file).replace('.edf', '').replace('.bdf', '')}_epoch_{i + 1}_{sanitized_event_name}_psd_topomap.png"
            output_path = os.path.join(subfolder_path, output_filename)
            fig.savefig(output_path)
            plt.close(fig)  # Close the figure to free up memory
            print(f"Saved PSD plot for epoch {i + 1} ({sanitized_event_name}) of {edf_file} to {output_path}")
