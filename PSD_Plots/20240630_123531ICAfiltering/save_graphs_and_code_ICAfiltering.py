...
def plot_psd(edf_file, output_directory):
    try:
        ...
        fig_before = raw.compute_psd(picks=eeg_channels, fmin=0.5, fmax=40).plot(show=False)
        ...
        fig_after = raw_clean.compute_psd(picks=eeg_channels, fmin=0.5, fmax=40).plot(show=False)
        ...
        for i, event in enumerate(events):
            ...
            fig = cropped_raw.compute_psd(picks=eeg_channels, fmin=0.5, fmax=40).plot(show=False)
...
if __name__ == '__main__':
    parent_directory = 'EDF+\Emotion Recognition'
