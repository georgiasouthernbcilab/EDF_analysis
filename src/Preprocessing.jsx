    ...
    return (
        <>
         <div className = "flex flex-col items-center mt-4">
            ...
            <p className = "bg-[#FFF6F6] rounded p-2 break-words mt-10 mb-5">Preprocessing steps can vary depending on your needs, but for general EEG cleanup there are some steps that should always be taken. First, we should import all the essentials for data preprocessing. For the sake of simplicity you can search for how to install these libraries.</p> 
...
import numpy as np`}
            ...
            <p className = "bg-[#FFF6F6] rounded p-2 break-words mt-10 mb-5">After installing the libraries, we create a list of all the EEG channels and the specific EOG channels. In this case, we are familiar with sites Fp1 and Fp2 being most closely associated with ocular information.</p>
            <pre><code className="language-python">
...
eog_channels=['Fp1', 'Fp2']`}
            ...
            <p className = "bg-[#FFF6F6] rounded p-2 break-words mt-10 mb-5">After establishing the sites we&apos;re going to analyze, we can start utilizing MNE-Python. They have a powerful function to read the EDF format which is much more versatile than the CSV alternative.</p>
            <pre><code className="language-python">
            {
                `# Read your EDF file
...
raw_clean = ica.apply(raw.copy())`}</code></pre>
                ...
                <p className = "bg-[#FFF6F6] rounded p-2 break-words mt-10 mb-5">Now we will perform power spectral density analysis on our data. Use <span className = "text-red-50 bg-[#282c34] p-1 rounded">plt.show()</span> to ensure the plot does not disappear.</p>
                <pre><code>{`raw.compute_psd().plot(picks="data", exclude="bads", amplitude=False)
plt.show()`}</code></pre>
                <p className = "bg-[#FFF6F6] rounded p-2 break-words mt-10 mb-5">That is the basics of preprocessing and using some of MNE-Python&apos;s suite of tools. They are powerful and versatile if you get familiar with them. Of course, this analyzes the entire recording. In most cases, we want to look at a more specific piece of time. We will explore this in Preprocessing 2.</p>
